...
class QuadTree
{
     ...
     **/
    public function search(QuadTreeBoundingBox $boundary, \SPLHeap $results) {
        if ($this->boundingBox->encompasses($boundary) || $this->boundingBox->intersects($boundary)) {
            ...
            if (isset($this->northWest)) {
                ...
                $this->northWest->search($boundary, $results);
                $this->northEast->search($boundary, $results);
                $this->southWest->search($boundary, $results);
                $this->southEast->search($boundary, $results);
            }
        }
    }
}
